...
VIT_DIR = os.path.join(BASE_DIR, "mi_vit_rocket_league")
VIT_BASE_PROCESSOR = "google/vit-base-patch16-224-in21k"
...
def cargar_modelos():
    ...
    try:
        mod_vit = ViTForImageClassification.from_pretrained(VIT_DIR)
    except OSError:
        st.warning(
            "No se encontraron pesos del modelo ViT en la carpeta local. "
            "Se usa el modelo base de ViT como respaldo."
        )
        mod_vit = ViTForImageClassification.from_pretrained(VIT_BASE_PROCESSOR)
    try:
        proc_vit = ViTImageProcessor.from_pretrained(VIT_DIR)
    except OSError:
        # Fallback when local training artifacts are incomplete (missing preprocessor_config.json).
        st.warning(
            "No se encontró 'preprocessor_config.json' en el modelo local. "
            "Se usa el procesador base de ViT como respaldo."
        )
        proc_vit = ViTImageProcessor.from_pretrained(VIT_BASE_PROCESSOR)
